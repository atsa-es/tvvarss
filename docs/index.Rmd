...
The `tvvarss` package has a function `simTVVAR` to simulate the process component of a TVVARSS model (_i.e._, it does not add observation error). We use that to simulate multiple realizations of a given process and then see how well the true parameters can be estimated.
...
```{r ex_1_plot_state, fig.height=4, fig.width=8}
...
```{r ex_1_lots, fig.height=4, fig.width=8}
ee <- 10
ex1 <- vector("list",ee)
for(i in 1:ee) {
...
```{r ex_1_screening, fig.height=4, fig.width=8}
