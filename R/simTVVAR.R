  ...
  function(B0,
    ...
    for (t in 1:TT + 1) {
      ...
      if (length(dim(B0)) == 2) {
        ...
        diag(BB[, , t]) <-
          plogis(qlogis(diag(BB[, , t - 1])) + diag(matrix(WW_BB[, t - 1], nn, nn)))
        ## constrain off-diag effects to [-1,1]
        BB[, , t][i_td] <-
          -plogis(qlogis(-BB[, , t - 1][i_td]) + WW_BB[i_td, t - 1])
        BB[, , t][i_bu] <-
          plogis(qlogis(BB[, , t - 1][i_bu]) + WW_BB[i_bu, t - 1])
        BB[, , t][i_cf] <-
          plogis(qlogis((BB[, , t - 1][i_cf] + 1) / 2) + WW_BB[i_cf, t - 1]) * 2 -
          1
      }
      ## state
      XX[, t] <-
        BB[, , t] %*% (XX[, t - 1, drop = FALSE] - CC %*% cc[, t - 1]) + CC %*% cc[, t] + WW_XX[, t -
            1]
    }
    return(list(
      B_mat = BB,
