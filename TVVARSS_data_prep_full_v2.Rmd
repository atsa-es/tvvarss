...
$$\mathbf{x}_{j,t} = \mathbf{u} + \mathbf{B}_t \left( \mathbf{x}_{j,t-1} - \mathbf{u} - \mathbf{C} \mathbf{c}_{j,t-1} \right) + \mathbf{C} \mathbf{c}_{j,t} + \mathbf{w}_{j,t}$$
...
When setting up the data below, it will be important to keep in mind the dimensions of the vectors and matrices for the equations above. Specifically, for $m$ species/guilds at $n$ sites/stations, with $q$ covariates and $T$ time points, we know:
...
* $\mathbf{u}$ is $\left[ m \times 1 \right]$;
...
ottr <- t(otters[,c("north","west","south")])
```
...
# variates don't start until period 2 of 1980, so remove period 1
anom34 <- anom34[-1,]
# rescale anomolies
...
### Observed versus estimated states
